...
def get_workshop_chart(data):
    ...
    status_data = {}
    for item in data:
        status_key = item.get("status") or "غير محدد"
        if status_key not in status_data:
            status_data[status_key] = 0
        status_data[status_key] += 1
...
def get_workshop_summary(data):
    ...
    # أنواع مختلفة
    brands = len(set(item.get("item_group") for item in data if item.get("item_group")))
